...
_Actually, the prompt said "Coach has read-only access to workouts. Code based invites." in the suggestions, but the user Selected "Coach can view all data... Email invites. No limits." BUT then in the follow-up text said "We don't support e-mail sending yet, what are your suggestions for 'connecting'?"_
...
To allow a Coach to view the App _as_ the Athlete, we need a mechanism to switch the data context without logging out.
