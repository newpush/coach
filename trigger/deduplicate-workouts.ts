          ...
          for (const field of mergeFields) {
            ...
            if (
              ...
              const donor = duplicatesList.find(
                (w) =>
                  w[field] !== null && w[field] !== undefined && w[field] !== 0 && w[field] !== ''
              )
              if (donor) {
                updates[field] = donor[field]
                logger.log(
                  `Merging field ${field} from duplicate ${donor.id} to ${bestWorkout.id}`,
                  {
                    value: donor[field]
                  }
                )
...
function areDuplicates(w1: any, w2: any): boolean {
  ...
  if (timeDiff > maxTimeDiff) {
    ...
    if (timeDiff < 2 * 60 * 60 * 1000) {
      logger.log(
        `Time diff too large: ${timeDiff}ms (${(timeDiff / (60 * 1000)).toFixed(1)}min) for workouts`,
        {
