  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="error" class="text-center py-12">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8 max-w-md mx-auto">
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-6">{{ error.message || 'This shared workout link is invalid or expired.' }}</p>
    ...
    <div v-else-if="workout" class="space-y-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
        ...
        <div class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div class="rounded-lg p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <div class="text-xs text-blue-600 dark:text-blue-400 mb-1">Planned Duration</div>
            <div class="text-xl font-bold text-blue-900 dark:text-blue-100">{{ formatDuration(workout.durationSec) }}</div>
          </div>
          <div v-if="workout.tss" class="rounded-lg p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
            <div class="text-xs text-amber-600 dark:text-amber-400 mb-1">Training Stress</div>
            <div class="text-xl font-bold text-amber-900 dark:text-amber-100">{{ Math.round(workout.tss) }}</div>
          </div>
          <div v-if="workout.workIntensity" class="rounded-lg p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
            <div class="text-xs text-green-600 dark:text-green-400 mb-1">Intensity</div>
            <div class="text-xl font-bold text-green-900 dark:text-green-100">{{ Math.round((workout.workIntensity || 0) * 100) }}%</div>
          </div>
        </div>
      ...
      <!-- Coach Instructions -->
      <div v-if="workout.structuredWorkout?.coachInstructions" class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-6 border border-blue-100 dark:border-blue-800">
      ...
      <div v-if="workout.structuredWorkout" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div v-if="workout.structuredWorkout?.messages?.length" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Timeline & Instructions</h3>
...
const pageDescription = computed(() => {
  if (workout.value) {
    ...
    return `Check out this planned ${workout.value.type || ''} workout: ${workout.value.title} (${dateStr}).`.substring(0, 160)
  }
