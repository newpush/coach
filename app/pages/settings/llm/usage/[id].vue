    ...
    <template v-else-if="data">
      ...
      <UCard>
        ...
        <div v-if="data.entityType" class="mt-4 pt-4 border-t">
          ...
          <div v-if="data.entityId" class="text-xs text-muted font-mono">ID: {{ data.entityId }}</div>
        </div>
      </UCard>
      ...
      <UCard v-if="data.promptFull || data.promptPreview">
        ...
        <div class="p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-lg max-h-96 overflow-y-auto">
        ...
        <div v-if="!data.promptFull && data.promptPreview" class="mt-2 text-xs text-muted">
          Note: Only preview available (first 500 characters). Full prompts are stored for newer records.
        </div>
      </UCard>
      ...
      <UCard v-if="data.responseFull || data.responsePreview">
        ...
        <div class="p-4 bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800 rounded-lg max-h-96 overflow-y-auto">
          <pre class="text-sm whitespace-pre-wrap">{{ data.responseFull || data.responsePreview }}</pre>
        </div>
        <div v-if="!data.responseFull && data.responsePreview" class="mt-2 text-xs text-muted">
          Note: Only preview available (first 500 characters). Full responses are stored for newer records.
        </div>
      </UCard>
    </template>
