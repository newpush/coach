...
export const publishCommand = new Command('publish')
  ...
  .action(async (options) => {
    ...
    if (!['announcements', 'technical'].includes(type)) {
      console.error(chalk.red(`Error: Invalid type '${type}'. Must be 'announcements' or 'technical'.`));
      process.exit(1);
    }
    ...
    if (!webhookUrl && !options.dryRun) {
      console.error(chalk.red(`Error: Discord webhook URL for '${type}' not found in environment variables.`));
      console.error(chalk.yellow(`Please set DISCORD_WEBHOOK_${type.toUpperCase()} in your .env file.`));
      process.exit(1);
    }
    ...
    if (firstHeaderIndex !== -1 && secondHeaderIndex !== -1) {
        console.log(chalk.yellow('Detected multiple release headers. Extracting only the latest release for publication...'));
    ...
    try {
      for (const [index, chunk] of chunks.entries()) {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Discord API returned ${response.status}: ${response.statusText} - ${errorText}`);
        }
        
        console.log(chalk.green(`✓ Successfully published chunk ${index + 1}/${chunks.length} to Discord (${type} channel)`));
      }
